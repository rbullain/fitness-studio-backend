...
class ClassInstance(TimeStampedModel):
    ...
    class_description = models.ForeignKey('classes.ClassDescription', on_delete=models.CASCADE, related_name='classes', verbose_name=_('class description'))
    class_schedule = models.ForeignKey('classes.ClassSchedule', on_delete=models.CASCADE, null=True, blank=True, related_name='classes', verbose_name=_('class schedule'))
...
class ClassSchedule(TimeStampedModel):
    ...
    class_description = models.ForeignKey('classes.ClassDescription', on_delete=models.CASCADE, related_name='schedules', verbose_name=_('class description'))
...
class Participation(TimeStampedModel):
    """Participation model."""
    class_instance = models.ForeignKey('classes.ClassInstance', on_delete=models.CASCADE, related_name='participations', verbose_name=_('class'))
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='participations', verbose_name=_('user'))

    is_cancelled = models.BooleanField(_('is cancelled'), default=False)
    cancellation_datetime = models.DateTimeField(_('cancellation datetime'), null=True, blank=True)

    class Meta:
        ordering = ('-created', '-modified',)

    def __str__(self):
        return f"{self.user} to {self.class_instance}"
